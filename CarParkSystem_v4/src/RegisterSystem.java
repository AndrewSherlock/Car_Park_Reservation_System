 ...
 * @version 2.1
 * Date 3-17
 * This Class Assists Registering for the system
 *
 */
public class RegisterSystem extends JPanel implements ActionListener{
	...
	public RegisterSystem(boolean isAdminUser, JPanel cardsArea)
	{ // constructor
		this.isAdminUser = true; 
		this.card = (CardLayout)cardsArea.getLayout();
	...
	/**
	 * This method is controller for the register validation
	 * @param fields
	 */
	void validateUserInformation(String[] fields) 
	{
		boolean isValid = true;
		int currentCheck = 0;  // current state on our check
		int threshold = fields.length - 7;
		
					
		while(isValid && currentCheck < fields.length - threshold){ // if we still have valid information and are still checking in the array bounds
			switch(currentCheck){
		...
		if(isValid) // if all our information is valid, we start preparing to add to database
		{
			...
			if(fields.length == 9){
			isStaffMember = ((fields[8] ==  "true")? true : false );
			...
			if(!db.checkForUsernameConflicts(fields[0])) // if our database has no username conflicts
			{
				...
				addUserToDatabase(userInfo, db, isStaffMember); // method that addeds to handles database
			} else{
	 ...
	 * @param isStaff
	 */
	void addUserToDatabase(String[] userInformation, DatabaseController database, boolean isStaff)
	{
		boolean isSuccessful = database.addNewUserToDatabase(userInformation, isStaff); // boolean if we are successful adding to database
		if(isSuccessful)
		{
			...
			card.show(this.getParent(), "OpeningCard");
		} else{
