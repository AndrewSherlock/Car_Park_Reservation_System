 ...
 * @version 2.1
 * Date 3-17
 * This Class is establishs connection with the database
 *
 */
public class DatabaseController {
	...
	Connection connectToDatabase() // connects to our database
	{
		try {
			...
			System.err.println("Class not found"); // Error state
		}
	...
	public boolean addNewUserToDatabase(String[] information, boolean isStaff)
	{
		Connection databaseConnection = connectToDatabase(); // takes in an array of user information
		int userID = createAccountId(databaseConnection);
		...
		try { 
			...
			PreparedStatement userLevel;
			if(isStaff)
			{
				userLevel = databaseConnection.prepareStatement("INSERT INTO staff(staff_ID) VALUES(?)");
				userLevel.setInt(1, userID);
				userLevel.execute();
			} else{
				userLevel = databaseConnection.prepareStatement("INSERT INTO customer(client_ID) VALUES(?)");
				userLevel.setInt(1, userID);
				userLevel.execute();
			}
	...
	public boolean checkForStaffPrivilage(String userName)
	{
		try{
			Connection databaseConnection = connectToDatabase();
			PreparedStatement statement = databaseConnection.prepareStatement("SELECT account_ID FROM users WHERE username = ?"); // looks within database for match
			statement.setString(1, userName);
			String [][] results = QueryDatabase(statement); // results
			int id;
			...
			if(results.length != 1)
			{
				...
				id = Integer.parseInt(results[0][0]);
				PreparedStatement staffSearch = databaseConnection.prepareStatement("SELECT staff_ID FROM staff WHERE staff_ID = ?");
				staffSearch.setInt(1, id);
				results = QueryDatabase(staffSearch);
				if(results.length == 1)
				{
			...
			System.err.println("Something went wrong with the staff check");
		}		
		return false;
	}
}
